...
fo, s: real;
begin
fo :=power(x,3)+(-2)*power(x,2)+(1)*x+(6);
fun := fo;
end;
...
begin
var fo: real;
fo := (power(x,4))/4-2*(power(x,3))/3+((power(x,2))/2)+6*x;
fun1 := fo;
end;
...
begin
MaximizeWindow;
Writeln('Вычисление площади фигуры, ограниченной кривой 1 * x* 3 + (-2) * x* 2 + (1) * x + (6) и осью Ох (в положительной части по оси Оу)');
